...
void initialiseParseHelpers(){
    ...
    addRuletoGrammar(145,grammar, statement, (grammarItem []){if_else},1);
    // addRuletoGrammar(146,grammar, if_else, (grammarItem []){if_,BO,abExpr,B,then,if_stmt,endif},5);
    addRuletoGrammar(147,grammar, if_stmt, (grammarItem []){START,statements,END,else_stmt},4);
    addRuletoGrammar(148,grammar, if_stmt, (grammarItem []){statement,else_stmt},2);
    addRuletoGrammar(149,grammar, else_stmt, (grammarItem []){else_,else_stmts},2);
    addRuletoGrammar(150,grammar, else_stmts, (grammarItem []){START,statements,END},3);
    addRuletoGrammar(151,grammar, else_stmts, (grammarItem []){statement},1);
    addRuletoGrammar(152,grammar, else_stmt, (grammarItem []){EPS},1);
    addRuletoGrammar(146,grammar, if_else, (grammarItem []){if_,abExpr,then,if_stmt,endif},5);
...
treeNodePtr createParseTree(char* inputFile) {
    ...
    while(X != DUMMY) {
        if(isTerminal(X)) {
            if(X == a.gItem) {
               ...
               if(a.gItem == DUMMY) {
                   errFlag = 1;printf("Out of input characters to parse grammar");
                   return NULL;
               }
                if(!error){ 
                errFlag = 1;printf("\tSYNTAX ERROR AT LINE %d: MISSING TERMINAL %s\n" ,a.lineNum,grammarArr[X]);
                }
                st= pop(st);
                X = top(st);
                currNode = nextElement(currNode);
            ...
            if(a.gItem == DUMMY) {
                if(FirstSets[X-iterativeStmt][EPS-INTEGER] == true) {
                    ...
                    errFlag = 1;printf("Syntax error - Out of input chracters to parse grammar");
                    return NULL;
                }
            }
            else{
                    ...
                    if(currRule != NULL) {
                        ...
                        if(FollowSets[X-iterativeStmt][a.gItem-INTEGER]){
                            ...
                            currNode = nextElement(currNode);
                            if(!error){
                            errFlag = 1;printf("\tSYNTAX ERROR AT LINE %d: INVALID POSITION OF %s\n",a.lineNum,a.lexeme);}
                            error=true;
                        }
                        else{
                            st = push(X,st);
   ...
   if(a.gItem != DUMMY) {
       errFlag = 1;printf("SYNTAX ERROR! UNNECESSARY INPUT %s AT LINE %d\n",a.lexeme,a.lineNum);
       return NULL;
   }
...
int printParseTree(treeNodePtr parseTree, char* outputfile){
    if(errFlag == 0){
        printf("Compilation successful without syntax errors\n");
    }
    printf("\n\n--------------PRINTING PARSE TREE IN outputParseTree.txt---------------\n\n");
